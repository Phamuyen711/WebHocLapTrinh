...
@section('content')
    ...
    <ul class="v-lesson-list">
        <!-- Vòng lặp hiển thị các bài học Video -->
        ...
        <!-- [BẮT ĐẦU] HIỂN THỊ BÀI KIỂM TRA CỦA CHƯƠNG -->
        @if(isset($quizzes))
            @php
                // Tìm bài kiểm tra thuộc về chương này
                $chapterQuiz = $quizzes->where('chapter', (string)$chapterNumber)->first();
            @endphp
            
            @if($chapterQuiz)
                <li class="v-lesson-item">
                    <!-- Link dẫn sang trang làm bài kiểm tra -->
                    <a href="{{ route('user.quiz.show', $chapterQuiz->id) }}" class="v-lesson-link" style="border-top: 1px dashed var(--v-border);">
                        <div class="v-lesson-icon">
                            <!-- Icon bài kiểm tra (hình tờ giấy tick V) -->
                        ...
                        <div class="v-lesson-title" style="color: var(--v-accent);">
                            [Bài kiểm tra] {{ $chapterQuiz->title }}
                            
                            <!-- Hiển thị trạng thái Đã qua / Chưa đạt -->
                            @if(isset($passedQuizzes) && in_array($chapterQuiz->id, $passedQuizzes))
                                <span style="color: #28a745; font-size: 12px; margin-left: 5px; font-weight: bold;">(Đã qua)</span>
                            @else
                                <span style="color: #dc3545; font-size: 12px; margin-left: 5px;">(Chưa đạt)</span>
                            @endif
                        </div>
                    </a>
                </li>
            @endif
        @endif
        <!-- [KẾT THÚC] HIỂN THỊ BÀI KIỂM TRA -->
