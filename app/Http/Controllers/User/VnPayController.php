...
class VnPayController extends Controller
{
    public function processPayment(Request $request)
    {
        ...
        try {
            ...
            Log::info('Creating payment with status: waiting', [
                'course_id' => $course->id,
                'user_id' => $user->id,
            ...
            $payment = Payment::create([
                ...
                'status' => 'waiting', // Khớp với ENUM trong CSDL
                'transaction_code' => $transaction_code,
            ]);

            // Cấu hình VNPay (Đọc trực tiếp từ file .env)
            $vnp_TmnCode = env('VNPAY_TMN_CODE');
            $vnp_HashSecret = env('VNPAY_HASH_SECRET');
            $vnp_Url = env('VNPAY_URL');
            $vnp_Returnurl = env('VNPAY_RETURN_URL');
            $expire = date('YmdHis', strtotime('+15 minutes')); // Thời gian động
            ...
            $inputData = [
                ...
                "vnp_OrderInfo" => "ThanhToan_GD_" . $vnp_TxnRef, // Viết liền không dấu để tránh lỗi encode
                "vnp_OrderType" => "other",
            ...
            if ($vnp_HashSecret) {
                ...
                $vnp_Url .= '&vnp_SecureHash=' . $vnpSecureHash; // Thêm '&' vào trước
            }
    ...
    public function returnPayment(Request $request)
    {
        ...
        // Lấy và bỏ vnp_SecureHash cùng với vnp_SecureHashType (Bắt buộc để tránh mã 97)
        ...
        if ($secureHash === $vnp_SecureHash && $payment) {
            ...
            if ($inputData['vnp_ResponseCode'] == '00') {
                ...
                $months = (int) $payment->course->expiration_date;
                ...
                CourseEnrolled::updateOrCreate(
                    ...
                    [
                        ...
                        'status' => 'in_progess', // Sai chính tả theo đúng CSDL của bạn (in_progess)
                    ]
                );
                ...
                $payment->status = 'canceled'; // Khớp với ENUM (1 chữ L)
                $payment->save();
    ...
    public function ipn(Request $request)
    {
        ...
        $vnp_SecureHash = $inputData['vnp_SecureHash'] ?? '';
        unset($inputData['vnp_SecureHash']);
        unset($inputData['vnp_SecureHashType']); // Bắt buộc xóa biến này
        ...
        try {
            if ($secureHash === $vnp_SecureHash && $payment) {
                if ($payment->amount == $inputData['vnp_Amount'] / 100) {
                    if ($payment->status == 'waiting') { // Khớp trạng thái ban đầu
                        if ($inputData['vnp_ResponseCode'] == '00' && $inputData['vnp_TransactionStatus'] == '00') {
                            
                            $payment->status = 'success'; // Cập nhật đúng ENUM
                            ...
                            // Cập nhật hoặc tạo mới khóa học học viên
                            CourseEnrolled::updateOrCreate(
                                [
                                    'id_user' => $payment->id_user,     // Đã sửa tên cột
                                    'id_course' => $payment->id_course, // Đã sửa tên cột
                                ],
                                [
                                    'title_course' => $payment->course->title, // Bổ sung cột
                                    'expiration_date' => $expirationDate,      // Bổ sung cột
                                    'status' => 'in_progess',                  // Đã sửa ENUM
                                ]
                            );
                        Log::info('IPN: Payment ' . $payment->transaction_code . ' updated to success');
                            return response()->json([
                                'RspCode' => '00',
                                'Message' => 'Confirm Success',
                            ]);
                        } else {
                            $payment->status = 'canceled'; // Đã sửa ENUM
                            $payment->vnp_transaction_no = $inputData['vnp_TransactionNo'];
                            $payment->vnp_bank_code = $inputData['vnp_BankCode'];
                            $payment->vnp_response_code = $inputData['vnp_ResponseCode'];
                            $payment->save();

                            Log::info('IPN: Payment ' . $payment->transaction_code . ' updated to canceled');
                            return response()->json([
                                'RspCode' => '00',
                                'Message' => 'Confirm Success',
                            ]);
